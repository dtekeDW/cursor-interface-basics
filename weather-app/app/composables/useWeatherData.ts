...
const DEFAULT_HERO = 'https://images.pexels.com/photos/415580/pexels-photo-415580.jpeg'
...
export const useWeatherData = () => {
  const citiesState = useState<CityMeta[]>('weather-live-cities', () => [...DEFAULT_CITIES])
  const activeCityId = useState<CityId>('weather-live-active-city', () => DEFAULT_CITY.id)
  const snapshotByCity = useState<Record<CityId, ForecastCoreSnapshot | undefined>>('weather-live-snapshots', () => ({}))
  const pendingByCity = useState<Record<CityId, boolean>>('weather-live-pending', () => ({}))
  const errorByCity = useState<Record<CityId, string | undefined>>('weather-live-errors', () => ({}))
  const isInitialized = useState<boolean>('weather-live-is-initialized', () => false)
  ...
  const fetchCitiesWeather = async (cities: CityMeta[]) => {
  ...
  const ensureInitialized = async () => {
    ...
    const activeCity = citiesState.value.find(city => city.id === activeCityId.value)
    if (activeCity)
      await fetchForecastForCity(activeCity)

    const otherCities = citiesState.value.filter(city => city.id !== activeCityId.value)
    void fetchCitiesWeather(otherCities)
  }
  ...
  const isActiveCityLoading = computed(() => {
    ...
    return pendingByCity.value[city.id] || !snapshotByCity.value[city.id]
  })
  ...
  const activeCityError = computed(() => {
    const city = activeCity.value
    if (!city)
      return undefined

    return errorByCity.value[city.id]
  })
